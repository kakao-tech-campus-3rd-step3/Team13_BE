...
public class MmrUpdater {
    ...
    private static final Adjudicator<Long> adjudicator = new Adjudicator<>(
        config,
    ...
    public void update() {
        ...
        adjustments.forEach(
            adjustment -> {
                findMmrById(adjustment.playerId())
                    .ifPresent(mmr -> {
    ...
    private SimpleTeamResult<Long> getTeamResult(List<Mmr> mmrList, boolean isWinner) {
        List<SimplePlayerResult<Long>> playerResults = mmrList.stream()
            .map(Mmr::getSimplePlayerResult)
            .toList();
